#!/bin/bash
# =============================================================================
# run_mo_ukv_sagemaker.sh
# SageMaker Processing Job wrapper for demo_mo_ukv.py
# =============================================================================
...
OUTPUT_S3_PREFIX="${OUTPUT_S3_PREFIX:-s3://my-bucket/aws-site-archive/output/ukv}"
JOB_NAME="aws-ukv-$(date +%Y%m%d%H%M%S)"
...
processor.run(
    code="scripts/demo_mo_ukv.py",
    job_name="${JOB_NAME}",
    arguments=["--time", "${QUERY_TIME}", "--anon", "--outdir", "/opt/ml/processing/output"],
    outputs=[
        ProcessingOutput(
            output_name="figures",
